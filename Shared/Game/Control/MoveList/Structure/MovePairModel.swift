import Foundation
import Observation

@Observable
class MovePairModel : Equatable {
    let moveNumber:Int
    var white:MoveModel?
    var black:MoveModel?

    private init(moveNumber: Int, white: MoveModel? = nil, black: MoveModel? = nil) {
        self.moveNumber = moveNumber
        self.white = white
        self.black = black
    }

    func hasWhiteMoved() -> Bool {
        white != nil
    }

    func hasBlackMoved() -> Bool {
        black != nil
    }

    func hasVariations(_ color:PieceColor) -> Bool {
        switch color {
        case .white:
            return white?.getVariations().count ?? 0 > 0
        case .black:
            return black?.getVariations().count ?? 0 > 0
        }
    }

    static func == (lhs: MovePairModel, rhs: MovePairModel) -> Bool {
        lhs.moveNumber == rhs.moveNumber && lhs.white?.id == rhs.white?.id && lhs.black?.id == rhs.black?.id
    }

    static func create(_ container: MoveModel, moveNumber:Int = 1)  -> MovePairModel {
        var rowContainer:MovePairModel
        if container.color == .white {
            rowContainer = MovePairModel(moveNumber: moveNumber, white: container)
        } else {
            rowContainer = MovePairModel(moveNumber: moveNumber, black: container)
        }
        return rowContainer
    }

    static func create(moveNumber:Int) -> MovePairModel {
        return MovePairModel(moveNumber: moveNumber)
    }
}
