...
struct DrawConditionEvaluator {
    ...
    private static func onlyOneMinorPieceLeft(_ figures: [any ChessPiece], type: PieceType) -> Bool {
        return figures.count == 3 && figures.filter({ $0.type == type }).count == 1
    }

    private static func onlySameColorBishopsLeft(_ figures: [any ChessPiece]) -> Bool {
