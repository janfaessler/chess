import Foundation

enum MoveFactory {

    private static let whiteStartingRow = "1"
    private static let blackStartingRow = "8"
    private static let ShortCastleTargetFile = "g"
    private static let LongCastleTargetFile = "c"

    static func create(_ input: any StringProtocol, position: Position) -> Move? {
        let color = position.colorToMove
        let figures = position.figures
        if isCastlingMove(input) {
            return createCastlingMove(input, color: color, figures: figures, position: position)
        }
        if isNotAPawnMove(input) {
            return createPieceMove(input, color, figures, position)
        }
        return createPawnMove(input, color: color, figures: figures, position: position)
    }

    private static func createCastlingMove(_ input: any StringProtocol, color: PieceColor, figures: [any ChessPiece], position: Position) -> Move? {
        let kingRow = getRowOfKing(color)
        let kingFile = getFileOfKing(input)
        let fig = getFigure(targetField: "\(kingFile)\(kingRow)", type: .king, color: color, figures: figures, position: position)
        return fig?.createMove("\(kingFile)\(kingRow)", type: .castle)
    }

    private static func createPieceMove(_ input: any StringProtocol, _ color: PieceColor, _ figures: [any ChessPiece], _ position: Position) -> Move? {
        let cleanedInput = clean(input)
        guard
            let pieceEndIndex = getEndOfPieceIdentIndex(cleanedInput),
            let pieceType = getPieceType(cleanedInput[..<pieceEndIndex]),
            let field = getPieceField(cleanedInput)
        else { return nil }
        let fig = getPieceFigure(cleanedInput, pieceType: pieceType, color: color, figures: figures, position: position)
        return fig?.createMove(field)
    }

    private static func createPawnMove(_ input: any StringProtocol, color: PieceColor, figures: [any ChessPiece], position: Position) -> Move? {
        guard let field = getField(input) else { return nil }
        var fig: (any ChessPiece)?
        if hasFileInfo(clean(input)) {
            let fileInfo = String(input.first ?? " ")
            fig = getFigure(field, withFile: fileInfo, type: .pawn, color: color, figures: figures, position: position)
        } else {
            fig = getFigure(targetField: field, type: .pawn, color: color, figures: figures, position: position)
        }
        if isPromotion(input) {
            let promoteTo = getPromotToFigure(input)
            return fig?.createMove(field, type: .promotion, promoteTo: promoteTo)
        }
        return fig?.createMove(field)
    }

    private static func getPieceFigure(_ cleanedInput: any StringProtocol, pieceType: PieceType, color: PieceColor, figures: [any ChessPiece], position: Position) -> (any ChessPiece)? {
        guard let field = getPieceField(cleanedInput) else { return nil }
        if hasRowInfo(cleanedInput) {
            let rowInfo = getPiecePositionInfo(cleanedInput)
            return getFigure(field, withRow: rowInfo, type: pieceType, color: color, figures: figures, position: position)
        } else if hasFileInfo(cleanedInput) {
            let fileInfo = getPiecePositionInfo(cleanedInput)
            return getFigure(field, withFile: fileInfo, type: pieceType, color: color, figures: figures, position: position)
        } else {
            return getFigure(targetField: field, type: pieceType, color: color, figures: figures, position: position)
        }
    }

    private static func getPieceType(_ char: any StringProtocol) -> PieceType? {
        guard char.count == 1, let first = char.first,
              let type = PieceType(fenChar: first), type != .pawn else { return nil }
        return type
    }

    private static func getFigure(targetField: any StringProtocol, type: PieceType, color: PieceColor, figures: [any ChessPiece], position: Position) -> (any ChessPiece)? {
        let candidates = getFigures(targetField: targetField, type: type, color: color, figures: figures, position: position)
        guard candidates.count == 1 else { return nil }
        return candidates.first
    }

    private static func getFigure(_ field: any StringProtocol, withRow: String, type: PieceType, color: PieceColor, figures: [any ChessPiece], position: Position) -> (any ChessPiece)? {
        let candidates = getFigures(targetField: field, type: type, color: color, figures: figures, position: position).filter { $0.row == Int(withRow) }
        guard candidates.count == 1 else { return nil }
        return candidates.first
    }

    private static func getFigure(_ field: any StringProtocol, withFile: String, type: PieceType, color: PieceColor, figures: [any ChessPiece], position: Position) -> (any ChessPiece)? {
        let candidates = getFigures(targetField: field, type: type, color: color, figures: figures, position: position).filter { $0.square.fileName == withFile }
        guard candidates.count == 1 else { return nil }
        return candidates.first
    }

    private static func getFigures(targetField: any StringProtocol, type: PieceType, color: PieceColor, figures: [any ChessPiece], position: Position) -> [any ChessPiece] {
        let figuresOfTypeAndColor = figures.filter { $0.type == type && $0.color == color }
        return figuresOfTypeAndColor.filter {
            guard let move = $0.createMove(targetField) else { return false }
            return $0.isMovePossible(move, board: position)
        }
    }

    private static func getPieceField(_ input: any StringProtocol) -> String? {
        let hasFileInfo = hasFileInfo(input)
        let hasRowInfo = hasRowInfo(input)
        let offset = hasFileInfo || hasRowInfo ? 2 : 1
        guard let fieldStartIndex = input.index(input.startIndex, offsetBy: offset, limitedBy: input.endIndex) else { return nil }
        let promotionParts = input[fieldStartIndex...].split(separator: ChessNotation.promotion)
        guard let field = promotionParts.first else { return nil }
        return String(field)
    }

    private static func getField(_ input: any StringProtocol) -> (any StringProtocol)? {
        let input = String(input).replacing(ChessNotation.check, with: "")
        let captureParts = input.split(separator: ChessNotation.capture)
        let promotionParts = captureParts.last?.split(separator: ChessNotation.promotion)
        return promotionParts?.first
    }

    private static func isNotAPawnMove(_ input: any StringProtocol) -> Bool {
        return input.first?.isUppercase ?? false
    }

    private static func isPromotion(_ input: any StringProtocol) -> Bool {
        let captureParts = input.split(separator: ChessNotation.capture)
        guard let lastPart = captureParts.last else { return false }
        let promotionParts = lastPart.split(separator: ChessNotation.promotion)
        return promotionParts.count > 1
    }

    private static func getPromotToFigure(_ input: any StringProtocol) -> PromotionPiece {
        let promotionParts = input.split(separator: ChessNotation.promotion)
        guard let lastPart = promotionParts.last,
              let firstChar = clean(String(lastPart)).first,
              let piece = PromotionPiece(fenChar: firstChar) else { return .queen }
        return piece
    }

    private static func isCastlingMove(_ input: any StringProtocol) -> Bool {
        return String(input) == ChessNotation.queensideCastle || String(input) == ChessNotation.kingsideCastle
    }

    private static func getEndOfPieceIdentIndex(_ input: any StringProtocol) -> String.Index? {
        return input.index(input.startIndex, offsetBy: 1, limitedBy: input.endIndex)
    }

    private static func getRowOfKing(_ color: PieceColor) -> String {
        return color == .white ? whiteStartingRow : blackStartingRow
    }

    private static func getFileOfKing(_ input: any StringProtocol) -> String {
        return String(input) == ChessNotation.kingsideCastle ? ShortCastleTargetFile : LongCastleTargetFile
    }

    private static func hasFileInfo(_ input: any StringProtocol) -> Bool {
        return (input.filter({ $0.isLowercase }) as [Character]).count == 2
    }

    private static func hasRowInfo(_ input: any StringProtocol) -> Bool {
        return input.filter({ $0.isNumber }).count == 2
    }

    private static func getPiecePositionInfo(_ cleanedInput: any StringProtocol) -> String {
        return String(cleanedInput.dropFirst().first ?? " ")
    }

    private static func clean(_ input: any StringProtocol) -> String {
        return String(input.filter({ $0 != ChessNotation.check && $0 != ChessNotation.checkmate && $0 != ChessNotation.capture }))
    }
}
