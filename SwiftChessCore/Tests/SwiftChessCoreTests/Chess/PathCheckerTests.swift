...
struct PathCheckerTests {
    ...
    @Test func testRow_backwardDirection_blockerBeforeTarget_returnsBlocker() throws {
        let rook = Piece.create(type: .rook, color: .white, row: 1, file: 8)
        let blocker = Piece.create(type: .pawn, color: .black, row: 1, file: 5)
    ...
    @Test func testDiagonal_oneStep_returnsTargetPiece() throws {
        let bishop = Piece.create(type: .bishop, color: .white, row: 1, file: 1)
        let target = Piece.create(type: .pawn, color: .black, row: 2, file: 2)
