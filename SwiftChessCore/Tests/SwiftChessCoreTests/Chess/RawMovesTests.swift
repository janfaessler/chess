...
final class RawMovesTests: ChessTestBase {
    ...
    @Test func testSlidingMoves_countMatches() throws {
        ...
        for moves in [rookMoves, bishopMoves, queenMoves] {
            ...
            #expect(Set(moves.map { $0.field }).count == moves.count, "no duplicate target squares")
        }

        #expect(bishopMoves.contains { $0.field == Field(row: 8, file: 8) }, "bishop reaches h8")
        #expect(bishopMoves.contains { $0.field == Field(row: 1, file: 1) }, "bishop reaches a1")
        #expect(rookMoves.contains { $0.field == Field(row: 4, file: 8) }, "rook reaches h4")
        #expect(rookMoves.contains { $0.field == Field(row: 1, file: 4) }, "rook reaches d1")
    }
    ...
    @Test func testPawnCannotCaptureOwnPiece() throws {
        ...
        let pawn = try #require(testee.figures.first { $0.equals(Figure.create("e4", type: .pawn, color: .white)!) })
    ...
    @Test func testStraightPawnPromotion() throws {
        ...
        let pawn = try #require(testee.figures.first { $0.equals(Figure.create("a7", type: .pawn, color: .white)!) })
        ...
        let move = try #require(Move("a8", piece: Figure.create("a7", type: .pawn, color: .white)!, type: .promotion))
    ...
    @Test func testPinnedPieceCannotMove() throws {
        ...
        let testee = try #require(testee)
        let bishop = try #require(testee.figures.first { $0.equals(Figure.create("e4", type: .bishop, color: .white)!) })
